...
$(GO_PATH)/bin/go-junit-report:
	@GO111MODULE=on go get -u -v github.com/jstemmer/go-junit-report@master
...
coverage/ci:  ## Takes packages test coverage, and output coverage results to CI artifacts.
	...
	GO111MODULE=on CGO_ENABLED=$(CGO_ENABLED) $(GO_TEST) -a -v $(strip $(GO_FLAGS)) -covermode=atomic -coverpkg=$(PKG)/... -coverprofile=${GO_TEST_COVERAGE_OUT} $(GO_PACKAGES) 2>&1 | tee /dev/stderr | go-junit-report -set-exit-code > /tmp/ci/test-results/junit.xml
...
$(GO_PATH)/bin/golangci-lint:
	@GO111MODULE=on go get -u -v github.com/golangci/golangci-lint/cmd/golangci-lint@master
